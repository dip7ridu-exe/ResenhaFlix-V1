...
const cfg={
  frost:savedStreams,
  meta:normalizeCatalogManifest(localStorage.getItem("cf2_meta")||CFG_DEFAULT.meta),
...
if(cfg.meta===CINEMETA_MANIFEST)cfg.meta=TMDB_PTBR_MANIFEST;
cfg.lang="pt-BR";
...
function configuredCatalogManifests(){
 ...
 return [...new Set([...REQUIRED_CATALOG_MANIFESTS,...saved.map(normalizeCatalogManifest)])]
}
...
async function getCatalog(manifest,type,id,params=""){
 ...
 const load=async(source)=>{
  const tmdb=isTmdbManifest(source),catalogId=tmdb&&id==="top"?"tmdb.top":!tmdb&&id==="tmdb.top"?"top":id;
  const data=await getJSONTimeout(catalogURLFor(source,type,catalogId,tmdb?catalogParamsPT(type,params):params),tmdb?5000:6000);
  return (data.metas||[]).map(m=>normalizeVideoMeta(m,type,source)).filter(Boolean).map(m=>{rememberPortuguese(m);return cachedPortuguese(m)});
 };
 const p=load(manifest).catch(async error=>{
  S.catalogCache.delete(key);
  if(isTmdbManifest(manifest)&&["top","tmdb.top"].includes(id))return load(CINEMETA_MANIFEST);
  throw error;
 });
...
async function resolveStreamId(type,id,meta){
 ...
 const parsed=raw.match(/^(tmdb:\d+|tt\d+)(:.*)?$/i);
 if(!parsed)return raw;
 const root=parsed[1],suffix=parsed[2]||"",key=type+"|"+root;
 if(isImdbId(root))return raw;
 let imdb=RF_ID_CACHE.get(key)||mediaImdbId(meta);
 if(!imdb){try{imdb=mediaImdbId((await getMetadata(type,root)).meta)}catch{}}
 if(imdb){RF_ID_CACHE.set(key,imdb);return imdb+suffix}
 return raw;
}
...
async function fetchPreviewMeta(type,id,token){
 try{
  ...
  if(!m||!("description" in m)||m._metadataLanguage!=="pt-BR"){
   const d=await getMetadata(type,id);
   m=d.meta||d;
...
function cleanMeta(m,sourceName){
 ...
 return {...cachedPortuguese(m),type:m.type||"movie",_catalogSource:sourceName||m._catalogSource||""};
}
function dedupeMetas(items){
 ...
 for(const raw of items){
  ...
  if(!map.has(key))map.set(key,m);
   ...
   if(old._metadataLanguage==="pt-BR"&&m._metadataLanguage!=="pt-BR"){map.set(key,{...m,...old});continue;}
...
async function catalogDefinitions(){
 const urls=configuredCatalogManifests(),key=urls.join("|");
 if(PT_DEFINITIONS&&PT_DEFINITION_KEY===key)return PT_DEFINITIONS;
 PT_DEFINITION_KEY=key;
 PT_DEFINITIONS=Promise.all(urls.map(async manifestUrl=>{
  try{
   const mf=await getManifest(manifestUrl);
   return (mf.catalogs||[]).filter(c=>["movie","series"].includes(c.type)).map(c=>({manifestUrl,manifest:mf,sourceName:isTmdbManifest(manifestUrl)?"TMDB":mf.name||new URL(manifestUrl).hostname,catalog:c}));
  }catch{return []}
 })).then(groups=>{
  const defs=groups.flat();
  for(const type of ["movie","series"]){
   if(!defs.some(d=>isTmdbManifest(d.manifestUrl)&&d.catalog.type===type&&catalogSupportsSearch(d.catalog)))defs.unshift({manifestUrl:TMDB_PTBR_MANIFEST,sourceName:"TMDB",catalog:{type,id:"tmdb.top",extra:[{name:"search"}]}});
   if(!defs.some(d=>d.manifestUrl===CINEMETA_MANIFEST&&d.catalog.type===type))defs.push({manifestUrl:CINEMETA_MANIFEST,sourceName:"Cinemeta",catalog:{type,id:"top",extra:[{name:"search"}]}});
  }
  return defs;
 });
...
async function searchAllCatalogs(q){
 const defs=await catalogDefinitions(),chosen=[],seen=new Set();
 for(const d of defs){
  if(!catalogSupportsSearch(d.catalog))continue;
  const key=d.manifestUrl+"|"+d.catalog.type;if(seen.has(key))continue;
  seen.add(key);chosen.push(d);
 }
 for(const d of defs){
  const key=d.manifestUrl+"|"+d.catalog.type;
  if(seen.has(key)||catalogNeedsOtherRequiredExtra(d.catalog))continue;
  seen.add(key);chosen.push(d);
 }
 const settled=await Promise.allSettled(chosen.map(async d=>{
  const searchable=catalogSupportsSearch(d.catalog);
  let items=await safeCatalog(d.catalog.type,d.catalog.id,searchable?{search:q}:"",d.manifestUrl);
  if(!searchable)items=items.filter(m=>normText(m.name).includes(normText(q)));
  return items.map(m=>cleanMeta(m,d.sourceName)).filter(Boolean);
 }));
 const items=dedupeMetas(settled.flatMap(r=>r.status==="fulfilled"?r.value:[]));
 // Addons sem busca continuam pesquisáveis no catálogo já carregado, sem novas consultas por tecla.
 for(const promise of S.catalogCache.values()){
  promise.then(items=>items.forEach(m=>S.itemCache.set(m.type+"|"+m.id,m))).catch(()=>{});
 }
 const matches=[...S.itemCache.values()].filter(m=>normText(m.name).includes(normText(q)));
 return dedupeMetas([...items,...matches]).sort((a,b)=>searchScore(b,q)-searchScore(a,q));
}
async function externalCatalogRows(){
 const defs=(await catalogDefinitions()).filter(d=>d.manifestUrl!==cfg.meta&&!catalogNeedsOtherRequiredExtra(d.catalog)&&!d.catalog.extra?.some(x=>x.name==="search"&&x.isRequired));
 const chosen=[],counts=new Map();
 for(const d of defs.sort((a,b)=>Number(b.manifestUrl===NETFLIX_MANIFEST)-Number(a.manifestUrl===NETFLIX_MANIFEST))){
  const n=counts.get(d.manifestUrl)||0;if(n>=2||chosen.length>=5)continue;
  counts.set(d.manifestUrl,n+1);chosen.push(d);
 }
 const rows=await Promise.all(chosen.map(async d=>{
  const items=rotateFresh((await safeCatalog(d.catalog.type,d.catalog.id,"",d.manifestUrl)).map(m=>cleanMeta(m,d.sourceName)).filter(Boolean),"external-"+d.catalog.id).slice(0,18);
  return {title:localizedCatalogLabel(d),sub:d.sourceName,items};
 }));
...
async function home(){
 ...
 $("#page").classList.remove("searchPage","animePageModern");
...
function setHero(m){
 m=cachedPortuguese(m);
 if(m._metadataLanguage!=="pt-BR")getMetadata(m.type||"movie",m.id).then(({meta})=>{if(S.hero?.id===m.id&&S.currentPage==="home"&&meta._metadataLanguage==="pt-BR")setHero(meta)}).catch(()=>{});
...
async function openDetails(type,id){
 const token=S.detailToken=(S.detailToken||0)+1;
 ...
 try{
  const d=await getMetadata(type,id);if(token!==S.detailToken||!$("#detailModal").classList.contains("open"))return;const m=d.meta||d;m.type=type;S.current=m;
  const anime=isAnimeLike(m);
  ...
  if(token!==S.detailToken)return;console.error(e);$("#detailTitle").textContent="Não foi possível carregar";toast("Erro ao buscar os detalhes.")
 }
}
...
async function resumeFromHistoryKey(key){
 ...
 try{
  if(entry.type==="series"){
   const d=await getMetadata("series",entry.rootId||entry.id);
   const show=d.meta||d;
   const vids=playableSeriesEpisodes(show);
   ...
   try{const d=await getMetadata("movie",entry.rootId||entry.id);movie=d.meta||d}catch{}
...
async function startPartyContext(context){
 ...
 if(!media){const d=await getMetadata(wanted?.type||"movie",wanted?.rootId);media=d.meta||d}
 S.current=media;
...
async function openPartyMedia(media,playback={}){
 ...
 if(media.type==="series"){
  ...
  if(!show){const d=await getMetadata("series",media.rootId);show=d.meta||d}
  ...
  if(!movie){const d=await getMetadata("movie",media.rootId);movie=d.meta||d}
  S.current=movie;
...
function metaHasGenre(m,genre){
 ...
 const target=normText(portugueseGenre(genre));
 return (m.genres||m.genre||[]).some(g=>{const value=normText(portugueseGenre(g));return value===target||value.includes(target)||target.includes(value)});
}
...
function aniTrendCard(m,type){const title=m.title?.english||m.title?.romaji||m.title?.native||"Anime",cover=m.coverImage?.extraLarge||m.coverImage?.large||"";return`<article class="aniResultCard aniTrendCard" data-ani-trend-title="${esc(title)}"><div class="aniResultCover" style="background-image:url('${esc(cover)}')"><div class="aniCatalogBadge">Em alta</div></div><div class="aniResultBody"><div class="aniResultTitle">${esc(title)}</div><div class="aniResultMeta">${esc(type)}${m.averageScore?` • ★ ${(m.averageScore/10).toFixed(1)}`:""}</div><div class="aniResultActions"><button type="button" data-ani-search>Buscar no ResenhaFlix</button></div></div></article>`}
...
async function trendingPage(){
 ...
 try{
  const [movies,series,animeCatalog,aniAnime]=await Promise.all([freshCatalog("movie","top","",cfg.meta,"trend-movies"),freshCatalog("series","top","",cfg.meta,"trend-series"),freshCatalog("series","top",{genre:"Animation"},cfg.meta,"trend-anime-catalog"),aniListTrendingMedia("ANIME",14).catch(()=>[])]);
  ...
  $("#pageBody").innerHTML=`<div class="trendingShell"><section class="trendingHero"><div class="trendingHeroText"><div class="trendingEyebrow">RADAR RESENHAFLIX</div><h2>Em alta agora</h2><p>Filmes, séries e animes que estão ganhando atenção. O ranking usa catálogos atualizados e tendências do AniList; o Radar Social facilita conferir a repercussão no TikTok, Instagram e Google Trends.</p></div><div class="trendingUpdated">Atualizado ${updated}</div></section><section class="socialRadar"><div class="socialRadarHead"><div><h3>Radar social</h3><p>Confira a repercussão do título com um toque. O ResenhaFlix não inventa números de redes que não fornecem um feed público anônimo.</p></div><span class="socialRadarBadge">SOCIAL</span></div><div class="socialBuzzList" id="socialBuzzList">${buzz.map(socialBuzzCard).join("")}</div></section><section class="trendingSection"><div class="trendingSectionHead"><h3>Filmes</h3><p>Atualização automática</p></div><div class="trendingRow" id="trendMovies">${movies.slice(0,18).map(card).join("")}</div></section><section class="trendingSection"><div class="trendingSectionHead"><h3>Séries</h3><p>Atualização automática</p></div><div class="trendingRow" id="trendSeries">${series.slice(0,18).map(card).join("")}</div></section><section class="trendingSection"><div class="trendingSectionHead"><h3>Animes</h3><p>Tendências atuais</p></div>${aniAnime.length?`<div class="trendingRow" id="trendAniLive">${aniAnime.slice(0,14).map(x=>aniTrendCard(x,"Anime")).join("")}</div>`:`<div class="trendingRow" id="trendAnime">${anime.map(card).join("")}</div>`}</section></div>`;
  bindSocialBuzz($("#socialBuzzList"));bindCards($("#trendMovies"));bindCards($("#trendSeries"));if($("#trendAnime"))bindCards($("#trendAnime"));if($("#trendAniLive"))bindAniTrendCards($("#trendAniLive"));
...
function dlivrosSearchUrl(book){
 ...
 return title?"https://dlivros.com/Buscar?q="+encodeURIComponent(title):"https://dlivros.com/";
}
function verifiedDlivrosUrl(value){
 try{const u=new URL(String(value||""));return u.protocol==="https:"&&u.hostname==="dlivros.com"&&!u.username&&!u.password&&/^\/livro\/[a-z0-9-]+\/?$/i.test(u.pathname)?u.href:""}catch{return ""}
}
function dlivrosBookUrl(book){
 for(const value of [book?.dlivrosUrl,book?.externalUrl,book?.url]){const direct=verifiedDlivrosUrl(value);if(direct)return direct}
 const title=normText(book?.title).split(":")[0].trim(),author=normText(book?.authors);
 if(title==="phantastes"&&author.includes("george")&&author.includes("macdonald"))return "https://dlivros.com/livro/phantastes-george-macdonald";
 return dlivrosSearchUrl(book);
}
function openBookOnDlivros(book){
 window.open(dlivrosBookUrl(book),"_blank","noopener,noreferrer");
}

function bookCardHtml(b,i){
 const saved=bookSaved(b),read=bestBookRead(b),formats=bookFormatChoices(b),download=bestBookDownload(b),destination=dlivrosBookUrl(b),direct=!!verifiedDlivrosUrl(destination);
 return `<article class="bookCard">
  <div class="bookCover">${b.image?`<img src="${esc(b.image)}" alt="Capa de ${esc(b.title)}" loading="lazy" decoding="async" width="200" height="300">`:'<span class="bookCoverFallback" aria-hidden="true">▤</span>'}</div>
  <div class="bookInfo"><h3 class="bookTitle">${esc(b.title)}</h3><div class="bookAuthor">${esc(b.authors||"Autor não informado")}${b.year?` • ${esc(b.year)}`:""}</div>
   <div class="bookBadges"><span>${esc(b.source)}</span>${b.publicDomain?`<span class="free">${b.publicAccess?'Acesso público':'Domínio público'}</span>`:""}</div>
   ${b.description?`<p class="bookDescription">${esc(b.description)}</p>`:""}
   <div class="bookSourceLine">${formats.length?esc([...new Set(formats.map(x=>x.label))].join(" · ")):"Consulte a disponibilidade na fonte"}</div>
   ${b.publicDomain&&formats.length?`<div class="bookFormatRow">${formats.filter(x=>["pdf","epub","mobi"].includes(x.kind)).slice(0,4).map(x=>`<button type="button" class="${x.readable?"readable":""}" data-book-format="${i}" data-book-format-kind="${esc(x.kind)}" data-book-format-url="${esc(x.url)}">${esc(x.label)}</button>`).join("")}</div>`:""}
   <div class="bookActions">
    ${read&&b.publicDomain?`<button type="button" class="read" data-book-read="${i}">▶ Ler ${esc(read.label)}</button>`:""}
    ${download?`<button type="button" data-book-download="${i}">↓ Baixar</button>`:""}
    <a class="dlivrosBtn" href="${esc(destination)}" target="_blank" rel="noopener noreferrer" aria-label="${direct?'Abrir':'Procurar'} ${esc(b.title)} no dLivros (nova aba)">${direct?'Abrir no dLivros':'Procurar no dLivros'} ↗</a>
    ${b.externalUrl?`<button type="button" class="bookOriginal" data-book-open="${i}">Página original ↗</button>`:""}
    <button type="button" class="bookSave ${saved?"saved":""}" data-book-save="${i}" aria-pressed="${saved}">${saved?"✓ Estante":"＋ Estante"}</button>
   </div>
  </div></article>`;
}
function bindBookCards(root,items){
 ...
 root.querySelectorAll("[data-book-save]").forEach(b=>b.onclick=()=>{const x=items[Number(b.dataset.bookSave)];toggleBookSaved(x);b.textContent=bookSaved(x)?"✓ Estante":"＋ Estante";b.classList.toggle("saved",bookSaved(x));b.setAttribute("aria-pressed",String(bookSaved(x)));if(S.booksTab==="library"&&!bookSaved(x))runBookSearch(S.booksQuery)})
}
async function runBookSearch(q=S.booksQuery){
 const root=$("#bookResults");if(!root)return;
 const token=S.bookSearchToken=(S.bookSearchToken||0)+1,tab=S.booksTab;
 S.booksQuery=String(q||"").trim();const query=S.booksQuery;
 const current=()=>S.currentPage==="books"&&root.isConnected&&S.bookSearchToken===token;
 const render=items=>{
  if(!current())return;
  S.bookResults=items;
  $("#bookResultMeta").textContent=`${items.length} ${items.length===1?'livro':'livros'} · ${tab==="library"?"Minha estante":tab==="free"?"Grátis":"Explorar"}`;
  root.innerHTML=items.length?`<div class="bookGrid">${items.map(bookCardHtml).join("")}</div>`:`<div class="mediaEmpty"><b>${tab==="library"?"Sua estante está vazia.":"Nenhum livro encontrado."}</b><p>${tab==="library"?"Adicione livros usando o botão ＋ Estante.":"Tente outro título ou autor."}</p></div>`;
  if(items.length){bindBookCards(root,items);runWhenIdle(()=>hydrateBookFormatsProgressively(items,root))}
 };
 if(tab==="library"){render(bookLibrary().filter(b=>!query||normText([b.title,b.authors].join(" ")).includes(normText(query))));return}
 if(tab!=="free"&&query.length<2){root.innerHTML='<div class="mediaEmpty"><b>Qual vai ser sua próxima leitura?</b><p>Pesquise por título ou autor, escolha uma categoria ou explore os livros grátis.</p></div>';$("#bookResultMeta").textContent="";return}
 root.innerHTML='<div class="loading" role="status">Buscando livros…</div>';
 const jobs=tab==="free"?[searchGutendexBooks(query),customBookResults(query)]:[searchGutendexBooks(query),searchOpenLibraryBooks(query),customBookResults(query)];
 const results=await Promise.allSettled(jobs);
 if(!current())return;
 let items=dedupeBooks(results.flatMap(r=>r.status==="fulfilled"?r.value:[]));
 if(tab==="free")items=items.filter(b=>b.publicDomain);
 render(items);
 if(results.some(r=>r.status==="rejected"))$("#bookResultMeta").textContent+=" · Algumas fontes não responderam";
}
...
async function booksPage(){
 ...
 $("#page").classList.remove("searchPage",);$("#page").classList.add("booksPageModern");
 ...
 $("#pageBody").innerHTML=`<div class="mediaHub">
  <div class="mediaHubHero"><div class="mediaHubTitle"><small>LIVROS</small><h2>Sua próxima leitura</h2><p>Encontre livros em português e guarde seus favoritos na estante.</p></div><button type="button" class="mediaSourcesBtn" id="openBookSources">⚙ Fontes</button></div>
  <div class="mediaSearchBar"><input id="bookSearchInput" type="search" aria-label="Buscar livro por título ou autor" placeholder="Busque um título ou autor…" value="${esc(S.booksQuery)}" autocomplete="off"><button id="bookSearchBtn">Buscar</button></div>
  ...
  <div class="mediaResultMeta" id="bookResultMeta" role="status" aria-live="polite"></div><div id="bookResults"></div>
 </div>`;
...
$("#settingsBtn").onclick=()=>{$("#frostUrl").value=cfg.frost;$("#metaUrl").value=cfg.meta;$("#catalogUrls").value=configuredCatalogManifests().join("\n");$("#subtitleAddon").value=cfg.subtitleAddon;$("#audioPref").value=cfg.audioPref;$("#subtitlePref").value=cfg.subtitlePref;$("#skipIntroEnabled").value=S.skipIntroEnabled?"1":"0";if($("#corsProxyUrl"))$("#corsProxyUrl").value=localStorage.getItem("rf40_cors_proxy")||"";$("#lang").value=cfg.lang;$("#settingsModal").classList.add("open");document.body.classList.add("settingsOpen");openSettingsTab("geral")};
...
$("#saveSettings").onclick=()=>{cfg.frost=sanitizeStreamManifests($("#frostUrl").value.trim()||CFG_DEFAULT.frost).join("\n");cfg.meta=normalizeCatalogManifest($("#metaUrl").value.trim()||CFG_DEFAULT.meta);cfg.catalogs=$("#catalogUrls").value.trim()||CFG_DEFAULT.catalogs;cfg.subtitleAddon=$("#subtitleAddon").value.trim()||CFG_DEFAULT.subtitleAddon;cfg.audioPref=$("#audioPref").value;cfg.subtitlePref=$("#subtitlePref").value;setSkipIntroEnabled($("#skipIntroEnabled").value!=="0",{notify:false});cfg.lang="pt-BR";localStorage.setItem("cf2_frost",cfg.frost);localStorage.setItem("cf2_meta",cfg.meta);localStorage.setItem("cf4_catalogs",cfg.catalogs);localStorage.setItem("cf5_subtitle_addon",cfg.subtitleAddon);localStorage.setItem("cf5_audio_pref",cfg.audioPref);localStorage.setItem("cf5_subtitle_pref",cfg.subtitlePref);localStorage.setItem("rf40_cors_proxy",($("#corsProxyUrl")?.value||"").trim());localStorage.setItem("cf2_lang",cfg.lang);S.manifestCache.clear();S.catalogCache.clear();clearPortugueseRequests();$("#settingsModal").classList.remove("open");document.body.classList.remove("settingsOpen");toast("Configurações salvas.");home()};
