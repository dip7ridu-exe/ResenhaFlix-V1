...
## Atualização v62 — livros e catálogos PT-BR

A aba de mangás e seu motor, leitor, estilos, testes e infraestrutura foram removidos. Filmes, séries, animes, player e Sala Resenha permanecem.

Os livros usam a paleta do site, capas com carregamento adiado e botões de leitura, download e estante. Todos oferecem acesso ao dLivros: links específicos informados no campo `dlivrosUrl` abrem a obra; os demais abrem a busca interna por título, sem passar pelo Google. O exemplo de Phantastes abre sua página confirmada.

O TMDB fornece o catálogo principal e metadados em PT-BR; o catálogo Netflix continua integrado. Buscas e metadados têm cache e consultas limitadas. Títulos salvos e de fontes secundárias recebem atualização de idioma quando ficam visíveis. O Cinemeta mantém uma alternativa quando o TMDB falha. Textos sem tradução oficial podem permanecer no idioma original. A integração não modifica áudio nem legenda do vídeo.

Consulte `LEIA-ME-ATUALIZACAO-v62.txt` para aplicar o pacote.
